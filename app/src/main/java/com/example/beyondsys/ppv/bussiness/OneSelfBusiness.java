...
public class OneSelfBusiness {
    ...
    /*提交个人信息*/
    public void Updateinformation(final Handler handler,ACache mCache){
        Informationperson person =new Informationperson();
        UserLoginResultEntity userLoginResultEntity=(UserLoginResultEntity)mCache.getAsObject(LocalDataLabel.Proof);
        person.proof=userLoginResultEntity.Proof;
        PersonInfoEntity personInfoEntity=(PersonInfoEntity)mCache.getAsObject(LocalDataLabel.PersonalInformation);
        person.Name=personInfoEntity.Name;
        person.Email=personInfoEntity.EMail;
        person.IDCard=personInfoEntity.IDNo;
        person.Address=personInfoEntity.Address;
        person.Remark=personInfoEntity.Remark;
        String JsonParams= GsonUtil.getGson().toJson(userLoginResultEntity.Proof);
        final JSONObject postJson= JsonEntity.Getjson("7", JsonParams);
        Log.i("Post:" + postJson, "TAG");
        new Thread(){
            public void run(){
                HttpURLConnection connection = null;
